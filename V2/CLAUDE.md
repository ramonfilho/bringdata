...
**Modelo pela mesma porta (desde 14/09/2026).** `bash V2/scripts/abrir_pr_modelo.sh <run_id>` ativa o run no YAML de produção (preservando o bloco do A/B), gera o model card no topo do `docs/MODEL_CHANGELOG.md` e abre a PR. O gate do modelo (`V2/scripts/ci_check_active_model.py`) julga só o run que entra: FINISHED no MLflow, artefatos no bucket, pisos de AUC e monotonia de `configs/retreino_mensal.yaml`, lineage (commit gravado, árvore limpa) e, para champion, delta contra o anterior. As métricas vêm do run do MLflow, o mesmo que gera o model card; o bloco `performance:` do YAML deixou de existir. A identidade do CI no GCP se cria uma vez com `bash V2/scripts/bootstrap_identidade_ci.sh` (IAM, só o dono).
