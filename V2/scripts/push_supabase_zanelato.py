...
def _avisa_no_dm(linhas_texto: list, resumo: str, poster=None, canal: str = "") -> dict:
    ...
    A implementação mudou de casa em 12/08/2026 para `scripts/aviso_dm.py`, porque a
    ingestão precisou do mesmo aviso e a alternativa era ela importar daqui — dependência ao
    contrário, já que é ESTE script que lê o que a ingestão escreve. O porquê do aviso existir
    está documentado lá.

    O nome fica: a `auditar()` chama por ele e os testes o substituem por atributo de módulo,
    então trocar o nome quebraria os dois sem nenhum ganho.
    ...
    return avisa_no_dm(linhas_texto, resumo, poster=poster, canal=canal)
