 ...
 * IMPORTANTE: Usa sendBeacon para garantir envio mesmo durante navegação/redirect
 */
function sendToCapiAPI(name, email, phone, hasComputer, utm, fbp, fbc, eventID, userAgent, eventSourceUrl) {
  ...
  // sendBeacon garante envio mesmo durante navegação/redirect
  ...
  if (success) {
    console.log('✅ CAPI enviado via sendBeacon');
  } else {
    ...
    fetch('https://bring-data-api-12955519745.us-central1.run.app/webhook/lead_capture', {
      ...
      keepalive: true  // Permite que a requisição continue mesmo durante navegação
    }).then(r => r.json())
      .then(data => console.log('✅ CAPI enviado via fetch:', data))
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (submitButton) {
    submitButton.addEventListener("click", function(event) {
      ...
      // Enviar para CAPI API (usa sendBeacon para garantir envio mesmo durante redirect)
