...
#### 3c — API multi-cliente (se Cliente B usa a API)

*Somente necessário se Client B for a produção via API. Pode ser paralelo ao 3b se Client B não usa API imediatamente.*

10. **`api/capi_integration.py`** — Pixel ID, event names, high_quality_decils (#103–#106) lidos de `CAPIConfig`.
11. **`api/business_config.py`** — taxas de conversão, ROAS targets (#90–#98) lidos do ClientConfig.
12. **`api/app.py`** — CORS origins, column_mapping, batch sizes, UTM filters (#109–#116) lidos do ClientConfig.
