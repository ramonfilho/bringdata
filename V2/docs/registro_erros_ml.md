...
- **T1-15** ✅ **Concluído (08/05/2026)** — nova função `audit_encoding_ab_variants` em [tests/parity_audit.py](../tests/parity_audit.py) itera por cada variante de `configs/active_models/{client}.yaml` aplicando `merge_encoding(base, variant.encoding_overrides)` antes de chamar `apply_encoding`. Smoke checks por variante: ordinais devem ter dtype numérico, sem NaN, nomes válidos. `deploy_capi.sh` Gate A agora chama `--function encoding_ab`. Limitação atual: smoke checks pegam a classe específica do bug Cluster 5, mas não comparam contra snapshot por-variante (precisa próximo retreino).
