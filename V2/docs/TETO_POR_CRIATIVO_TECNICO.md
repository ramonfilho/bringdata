...
| `public.scores_inbound` (Supabase da agência) | A ENTREGA: nota de qualidade + `teto_cpl`/`teto_roas_alvo`/`teto_referencia` por criativo e por campanha, 2 cortes (acumulado e 3 dias) | job `push-scores-zanelato` (hora em hora, :22) — `scripts/push_scores_zanelato.py` |
