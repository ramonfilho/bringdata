...
**Status:** 🟡 **I0–I4 (código) concluídos e commitados** no branch `feat/capi-lead-surveys-scoring` (ledger `registros_ml` criado/testado, adaptador I2, enriquecimento I3, ramo isolado I4; testes offline verdes; fluxo `Lead` byte-idêntico). ⏸️ **PAUSADO em 2026-05-19** a pedido do usuário, **antes do deploy canary do I4**. **Nada em produção** (`SURVEY_CAPI_ENABLED` off, sem canary, schedulers pausados). Falta: validação canary (fecha I4) → I5 (monitoramento + alarme rolling sobre `registros_ml`) → I6 (whitelist UTM) → I7 (ligar/promover/religar). Consolidado 2026-05-19 via `/docs`. Nota de prazo: estrutura nova do dono ("tudo do zero") prevista p/ "semana que vem" (17/05).
**Relação:** roadmap em `PLANO_EXECUCAO.md` ("Frente ativa urgente"). Monitoramento/`critical_alerts` pertence a **outro terminal** (commits `52b507d`/`642ffa4`/`51951ee` no `main`) — esta frente é só scoring/CAPI, isolada em worktree. Classe de quebra histórica em `AUDITORIA_QUEBRA_PRODUCAO.md`. Toca `/railway/process-pending` (`api/app.py`), reusa `api/railway_mapping.py`+`send_batch_events`. Front (colunas limpas, durabilidade): `instrucoes_dev_frontend_capi.md`. Regra fail-loud: `CLAUDE.md`.
