...
> **Reorganização (18/09/2026):** só ficam no topo de `docs/` os documentos que o README cita (gatekeeper, safeguard, changelog do modelo, treino contínuo, arquitetura e este índice) mais as pastas `relatorios/` e `arquivo/`. Todo o resto vive em `docs/interno/`; os nomes abaixo continuam valendo lá.
...
**Relação:** consome [FAN_OUT_CAPI.md](interno/FAN_OUT_CAPI.md) como artefato técnico vivo do mecanismo de fan-out (referência detalhada do laço dentro de `send_both_lead_events`). Preserva princípio "uma variante = um evento" do [AB_TEST.md](interno/AB_TEST.md). Toca caminhos de lead de [PROCESSO_CAPI_LEAD_SURVEYS.md](interno/PROCESSO_CAPI_LEAD_SURVEYS.md). Segue padrão Repositório + adaptadores de [REFATOR_MONITORAMENTO_CAMADA_ACESSO.md](interno/REFATOR_MONITORAMENTO_CAMADA_ACESSO.md). Frente analítica em worktree separado `bring_data-roas` (scripts em `V2/scripts/pull_roas_dataset.py`, `analise_roas_recalibracao.py`, `analise_roas_a_vista.py`, `validate_roas_analysis.py`); PDF para stakeholders em [propostas_e_apresentacoes/descoberta_roas_devclub.pdf](../propostas_e_apresentacoes/descoberta_roas_devclub.pdf). Roadmap em `PLANO_EXECUCAO.md` (gate único de validação OOS + pré-requisitos do segundo cliente entram aqui).

### `plano_decis_dois_modelos_ledger.md` 📋 Plano de refator (frente ativa)
**Papel:** plano para **aposentar o refresh diário e a tabela `scores_historicos`**, movendo o scoreamento pelos dois modelos (Champion + Challenger) para o momento do scoreamento online (consumer Pub/Sub) e gravando os dois decis + run_ids direto no ledger `registros_ml`. Elimina a corrida do painel de decis (o "Google 40%" falso das 06:00, diagnosticado em 05/07/2026), o estouro de memória do catch-up, o backfill diário e o join `scores_historicos ⋈ registros_ml`. Traz o desenho (`/sw-architect`: generalizar `score_lead_from_payload` pra emitir os dois, reusar o scorer em lote só pra backfill), o schema (`/data-architect`: 8 colunas novas, decil como INT, run_id por linha, metadados de lançamento deriváveis do calendário), as 5 fases de estrangulamento com rollback, o gate de cobertura antes do drop, e o porquê de o backfill passar a ser one-time por retreino em vez de diário.
**Status:** 📋 plano escrito 2026-07-05, direção B (ir direto ao refator, sem o stopgap de adiantar o refresh). Aguarda início da Fase 1 (schema additivo no `registros_ml`).
**Relação:** substitui a régua-única atual via refresh (`api/scores_refresh.py` → `scores_historicos`). Vizinho da dimensão de lançamento `analytics.launch_curation` (backlog) e da consolidação `analytics` ([CONSOLIDACAO_CLOUDSQL.md](interno/CONSOLIDACAO_CLOUDSQL.md)). Toca o hot path do consumer ([api/pubsub_branch.py](../api/pubsub_branch.py), [src/scoring/service.py](../src/scoring/service.py)) — exige parity audit + canary.
...
**Relação:** orquestrado por [EVENTOS_E_DECIS_PLANO.md](interno/EVENTOS_E_DECIS_PLANO.md) (este documento é o bloco A do plano unificado). Princípio "uma variante = um evento" preservado conforme [AB_TEST.md](interno/AB_TEST.md). Caminhos de entrada do lead que convergem em `send_both_lead_events` documentados em [PROCESSO_CAPI_LEAD_SURVEYS.md](interno/PROCESSO_CAPI_LEAD_SURVEYS.md). Artefatos vivos: [src/core/client_config.py](../src/core/client_config.py) (dataclass `ExtraHQDestination` e parser fail-loud), [configs/clients/devclub.yaml](../configs/clients/devclub.yaml) (bloco `extra_hq_destinations`), [api/capi_integration.py](../api/capi_integration.py) (laço dentro de `send_both_lead_events`).
