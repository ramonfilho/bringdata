...
**Relação:** substitui a régua-única atual via refresh (`api/scores_refresh.py` → `scores_historicos`). Vizinho da dimensão de lançamento `analytics.launch_curation` (backlog) e da consolidação `analytics` ([CONSOLIDACAO_CLOUDSQL.md](CONSOLIDACAO_CLOUDSQL.md)). Toca o hot path do consumer ([api/pubsub_branch.py](../api/pubsub_branch.py), [src/scoring/service.py](../src/scoring/service.py)) — exige parity audit + canary.
