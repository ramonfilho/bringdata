...
**Papel:** especificação **e diário** da frente que faz o evento CAPI scoreado por ML (`LeadQualified`/`LeadQualifiedHighQuality`) ser disparado também a partir da `lead_surveys` (a captação migrou de `Lead`→`lead_surveys` em ~12/05; o sinal scoreado ficou OFF para a inflow viva). Cobre o porquê (migração), a investigação (esteiras ~98% disjuntas; recuperação de UTM/fbp/fbc/`computador` via `UTMTracking`+`integration_logs`, `computador` ≈100% combinando n8n+activecampaign-144, validado 1011/1011), as decisões (recuperar de log já — não esperar coluna; recuperação 24h+forward; ledger próprio `registros_ml`; fluxo `Lead` intocado/isolado; fail-loud; restrições duras), o protocolo por item I0–I7 com status, o estudo de extensão de monitoramento (I5) e as pendências de go-live (whitelist de UTM — classe "Cluster 5").
**Status:** 🟡 **I0–I4 (código) concluídos e commitados** no branch `feat/capi-lead-surveys-scoring` (ledger `registros_ml` criado/testado, adaptador I2, enriquecimento I3, ramo isolado I4; testes offline verdes; fluxo `Lead` byte-idêntico). ⏸️ **PAUSADO em 2026-05-19** a pedido do usuário, **antes do deploy canary do I4**. **Nada em produção** (`SURVEY_CAPI_ENABLED` off, sem canary, schedulers pausados). Falta: validação canary (fecha I4) → I5 (monitoramento + alarme rolling sobre `registros_ml`) → I6 (whitelist UTM) → I7 (ligar/promover/religar). Consolidado 2026-05-19 via `/docs`. Nota de prazo: estrutura nova do dono ("tudo do zero") prevista p/ "semana que vem" (17/05). **Achado 2026-05-19:** o sistema novo já expõe `computador`/fbp/fbc/`eventId`/nome/telefone como **colunas limpas** na tabela viva `Client` (~100% p/ respondentes de pesquisa) — ao retomar, o enriquecimento por parse de log (I3) pode ser substituído por leitura direta do `Client`; código pausado não foi tocado.
**Relação:** roadmap em `PLANO_EXECUCAO.md` ("Frente ativa urgente"). Monitoramento/`critical_alerts` pertence a **outro terminal** (commits `52b507d`/`642ffa4`/`51951ee` no `main`) — esta frente é só scoring/CAPI, isolada em worktree. Classe de quebra histórica em `AUDITORIA_QUEBRA_PRODUCAO.md`. Toca `/railway/process-pending` (`api/app.py`), reusa `api/railway_mapping.py`+`send_batch_events`. Contrato de dados de fato = schema Railway (`Client`/`lead_surveys`/`UTMTracking`); `instrucoes_dev_frontend_capi.md` está **deprecado**. Regra fail-loud: `CLAUDE.md`.
...
**Status:** ⚠️ **DEPRECADO (2026-05-19).** O sistema novo foi reescrito; o contrato de dados de fato vive no schema do banco Railway (`Client`/`lead_surveys`/`UTMTracking`), não neste `.md`. Mantido só para referência histórica. Ver `PROCESSO_CAPI_LEAD_SURVEYS.md` (Achado 2026-05-19).
