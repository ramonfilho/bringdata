...
- ~~**Guard de coluna Medium em produção**~~ ✅ RESOLVIDO — guard `if 'Medium' in self.data.columns:` presente em [`src/production_pipeline.py:294`](../../src/production_pipeline.py#L294), espelhando o do treino.
- ~~**`/railway/process-pending` — `.str` accessor em batches de 1 lead**~~ ✅ RESOLVIDO — guards em [`src/core/utm.py:62-63`](../../src/core/utm.py#L62-L63) (`_src_notna` + `astype(str)` no Source) e [`src/core/utm.py:121`](../../src/core/utm.py#L121) (`.where(notna, '').astype(str)` no Term) eliminam o "Can only use .str accessor with string values" pra qualquer tipo inesperado vindo do JSONB.
...
- **F3** — `Term` `null_rate` em produção (14.3%) ultrapassou marginalmente o `max_null_rate` do schema (13.9%). **Decisão pendente:** investigar Meta Ads (`act_188005769808959`) se há ads/adsets ativos sem `utm_term` no `url_tags` → corrigir tracking, OU aumentar `max_null_rate` para ~0.18. **Impacto:** o `/monitoring/feature-report` retorna `severity=ERROR` em batches recentes e o Gate B do `deploy_capi.sh` bloqueia progressão do próximo deploy.
- **F5** ✅ RESOLVIDO 2026-05-14 — auditoria SQL nos últimos 30 dias (44.383 leads) mostra `Você já fez/faz/pretende fazer faculdade?` preenchida em 98.78% e vazia em só 1.22%. Schema atual permite 12.84% de null — produção bem abaixo. A pergunta deixou de ser opcional na prática; alerta F5 era stale.
...
- **E5** ✅ RESOLVIDO — `_check_score_distribution` em [`src/monitoring/data_quality.py:992-1051`](../../src/monitoring/data_quality.py#L992) agora implementa precedência: (1) rolling 30d em produção quando injetado pelo caller, (2) `model_metadata.json:decil_analysis` como fallback de treino, (3) hardcoded uniforme só como último recurso. Falso positivo crônico de D10 deixou de aparecer.
...
- **M1 — MIX QUENTE: decisão DEV-vs-LF REABERTA em 18/mai (hard-enforced no `--set-active`).** Premissa antiga (incluir como categoria canônica, "coluna OHE zerada em LF é neutra, zero routing") foi **revertida**: o público MIX QUENTE existe sempre em lançamento DEV e nunca em LF. Se o RF aprender um split condicionado em `Medium_MIX_QUENTE`, em LF todos os leads caem no mesmo galho da árvore → distorce a distribuição de decis em LF. Coluna OHE zerada **não** é neutra. Auditoria 2026-05-14: 14,87% do volume vira "Outros" hoje (≠ 7,3% histórico). **Decisão pendente entre 3 opções:** (A) `modelo_por_tipo_lancamento` — modelo DEV (com MIX QUENTE) separado do LF (sem), mais preciso e custo de 2 modelos; (B) `excluir` — não inclui, aceita ~15% do volume DEV em "Outros", mais simples e sem viés; (C) `incluir_e_auditar_decis_lf` — inclui como canônica MAS audita pós-treino se a presença/ausência distorce decis em LF. **Enforcement:** `retraining_decisions.mix_quente` no `configs/clients/devclub.yaml` está `PENDENTE`; o guard `_assert_retraining_decisions_resolved()` em `train_pipeline.py` **aborta `--set-active`** enquanto não for resolvido — impossível promover Champion novo sem decidir conscientemente. **Local de aplicação:** `configs/clients/devclub.yaml` (bloco `retraining_decisions` + `medium.category_mappings`).
...
- **M4 — DT-18 (normalizar 4 features binárias raw `genero`/`estudou_programacao`/`fez_faculdade`/`investiu_curso_online`) — hard-enforced no `--set-active`.** Prioridade ALTA. **Mecânica de transição segura (decidida 18/mai):** a normalização roda no `category_unification`, que é **global** (não per-variante). Logo, é **impossível** isolar por variante sem virar shim frágil. Regra: **NUNCA rodar A/B entre um modelo no formato antigo (`jan30`, sufixos crus `_Sim`/`_Não`) e um no formato novo (`_sim`/`_nao`)** — o lado antigo zera 100% das 4 features. Os dois lados do A/B têm que estar na mesma arquitetura. Caminho correto = **bundle imutável**: o próximo Champion **e** o próximo Challenger são treinados juntos já normalizados; no momento da promoção retreina-se o campeão (dados/hiperparâmetros novos), mede-se o deslocamento de performance — se pequeno, promove; se grande, regride — e o Challenger já entra na arquitetura nova (= Sprint 2 do `retraining_orchestrator`, quality gate + promoção condicional). **Enforcement:** `retraining_decisions.features_binarias_dt18` no `devclub.yaml` está `PENDENTE`; o guard em `train_pipeline.py` aborta `--set-active` até as 4 colunas estarem em `category.categorical_columns` E o próximo Challenger estar treinado normalizado (valor `aplicado_treino_e_producao`). Faz UM retreino só com DT-16+DT-18 (e DT-17 fases 4-5). Ver DT-18 em PLANO_REFACTOR_MLOPS.md e V.2 do `registro_erros_ml.md`.
...
- **🔴 PRIORIDADE MÁXIMA — P1: Implementar `audience_profile_drift` no monitoring (T1-13).** Descoberto em 08/05/2026 via análise ad-hoc do LF54 em captação: o perfil de público está significativamente deslocado do Top 5 ROAS histórico (recorte usado nessa análise: LF40, LF41, LF44, LF45, LF47, n=39.771 — Top 5 canonical atualizado em 2026-05-14 está em `docs/interno/METODOLOGIA_TOP5_ROAS.md` e é LF45, LF44, LF46, LF41, LF43) — "Sem computador" 12,5% → 22,9%, Feminino 18,3% → 28,0%, CLT/funcionário público 44,9% → 35,6%, "Já estudou programação Sim" 36,8% → 30,9% (todos com chi² p < 1e-4). Esse drift pode explicar parte do baixo desempenho dos LFs recentes e **deveria** ter sido detectado automaticamente pelo monitoring — não foi porque o `DataQualityMonitor.check_distribution_drift` em [src/monitoring/data_quality.py:397](../../src/monitoring/data_quality.py#L397) só compara contra `distribuicoes_esperadas.json` capturado **no treino**, não contra um perfil de "audiência winner" histórica. **Especificação técnica vai em PLANO_SAFEGUARD.md como T1-13** (catálogo). **Por que máxima prioridade:** gap deixou produção rodando às cegas para drift de público durante todos os LFs entre o último Top 5 ROAS e hoje; cada lançamento sem esse check é exposição cega; é também um erro registrado em `registro_erros_ml.md` § V.4. **Decisões já tomadas nesta sessão:** (a) referência **fixa** no Top 5 ROAS atual em snapshot estático (`configs/clients/devclub/reference_audience_profile.json`); refresh do pool é tarefa manual anexa a "fechamento de lançamento" — não é rolling; (b) janela comparada no alerta automático = **último dia completo** (BRT 00:00→23:59 anterior a hoje), NÃO o LF acumulado nem o dia parcial; (c) threshold = ⚠ se |Δpp| ≥ 5 em qualquer categoria canônica; severity HIGH se |Δpp| ≥ 5 em qualquer das 5 features socioeconômicas críticas (computador, gênero, ocupação CLT, programação, cartão); (d) categorias canônicas reaproveitadas da constante `UNIFICATION` em [scripts/perfil_audiencia.py](../../scripts/perfil_audiencia.py); (e) drift é pré-encoding e independente de modelo → NÃO precisa loop per-variant. **Artefatos já existentes (não bloqueiam, apoiam):** [docs/interno/perfil_audiencia_dev20.md](perfil_audiencia_dev20.md), [docs/interno/perfil_audiencia_lf54.md](perfil_audiencia_lf54.md), [scripts/perfil_audiencia.py](../../scripts/perfil_audiencia.py). **Esforço estimado:** ~1h implementação + revisão de threshold com 1-2 ciclos de produção. **Sem dependências bloqueadoras** — pode ser implementado imediatamente.

#### Sequelas / pendências da sessão de investigação 2026-06-07 — recalibração de decis por ROAS

Sessão de análise offline mostrou que reordenar os top-N leads por retorno esperado por real gasto (`(probabilidade × ticket à vista) ÷ custo_por_lead`) em vez de só probabilidade gera ganho líquido anual entre R$ 86 mil (top 10%) e R$ 298 mil (top 50%). A descoberta que destravou foi que o `utm_campaign` carrega o `campaign_id` no sufixo, permitindo atribuir custo por lead via Meta API com mediana R$ 5,35 e cobertura 82,8%. Análise paralela de calibração em [analise_calibracao_jan30_abr28.md](analise_calibracao_jan30_abr28.md) mostrou ECE pré-calibração de 26pp (Champion) e 40pp (Challenger) e razão de inflação 27-33× no D10 — calibração isotônica vira pré-requisito firme da fórmula em produção. Catálogo do "como" no plano integrador [EVENTOS_E_DECIS_PLANO.md](EVENTOS_E_DECIS_PLANO.md) (seis blocos: A fan-out atual já em canary, B lookup CPL, C interface DecileStrategy, D adapter `send_all_lead_events`, E observabilidade, F ativação RoasV1).

- **🟡 Bloco B — Lookup de custo por lead (independente).** Schema `cpl_adset` e `ad_to_adset_map` no **Railway** (ao lado de `registros_ml`); `CplRepository` (interface + Cloud SQL adapter + cache em memória) e `AdResolver` em `src/data/cost_attribution/`; job de refresh diário via Cloud Run Job + Cloud Scheduler puxando Meta Insights API por adset. Não toca caminho do lead. Catálogo: [EVENTOS_E_DECIS_PLANO.md](EVENTOS_E_DECIS_PLANO.md) §4 Bloco B. **Pré-condição:** nenhuma. **Pode rodar em paralelo a C.**
- **🟡 Bloco C — Interface `DecileStrategy` com paridade 100%.** Refator preparatório do scoring para aceitar múltiplas estratégias de decil. Define interface `DecileStrategy.assign(score, features, variant_config, lead_id) → DecileAssignment`; extrai a lógica atual de "decil + qual evento emitir" em `PropensityDecileStrategy`; teste de paridade contra produção (10k leads, 0 divergências). Catálogo: §4 Bloco C. **Pré-condição:** nenhuma. **Pode rodar em paralelo a B.**
- **🟡 Bloco E — Observabilidade unificada no `registros_ml`.** Migration aditiva: `decile_propensity`, `decile_roas_v1`, `cpl_source`, `events_fired`, `extra_hq_destinations_fired`. Resolve simultaneamente a pendência §11.2 do [FAN_OUT_CAPI.md](FAN_OUT_CAPI.md). Catálogo: §4 Bloco E. **Pré-condição:** C concluído.
- **🟡 Bloco D — Adapter `send_all_lead_events` envolvendo `send_both_lead_events`.** Novo caminho que itera sobre N atribuições e dispara o laço de fan-out existente por cada uma; `send_both_lead_events` vira adapter de 1 atribuição (paridade 100%). Catálogo: §4 Bloco D. **Pré-condição:** C concluído + bloco A (fan-out atual) promovido a 100% — toca o mesmo arquivo do canary `smart-ads-api-00680-jez`.
- **🟠 Bloco F — Ativação `RoasV1DecileStrategy` em paralelo.** Liga a recalibração via kill switch YAML; adiciona entrada nova em `extra_hq_destinations` pra cópia dos eventos novos no pixel destino do BM. Catálogo: §4 Bloco F. **Pré-condições:** B + D + E concluídos + **calibração isotônica entregue como pré-requisito não-negociável** (trabalho conduzido em terminal paralelo seguindo as conclusões da análise de calibração).
- **🔴 Bloqueante novo do bloco F — validação out-of-sample do calibrador isotônico (DT-20).** A análise de 08/05/2026 ([analise_calibracao_jan30_abr28.md](analise_calibracao_jan30_abr28.md)) mediu ECE pré-calibração de 26 pp (Champion) e 40 pp (Challenger), e a isotônica zera o gap in-sample por construção (limitação 1 declarada no doc). Falta confirmar que a calibração ajustada nos dados do treino original continua válida em leads recentes — drift de público pós-treino pode ter mudado a relação `score → P(real)`. **Trabalho:** pegar leads dos últimos 30-60 dias do Railway com label de conversão real, rodar `predict_proba` + isotônica fitada nos dados antigos, medir ECE residual em bins de 10 ou 20. **Critério:** ECE residual ≤ 5 pp → calibração generalizou, libera bloco F; entre 5-10 pp → avaliar custo/benefício e considerar recalibração periódica; > 10 pp → drift dominante, bloco F precisa de mecanismo de recalibração contínua antes. **Por que bloqueante e não apenas pré-condição:** sem essa medição, promover calibrador a produção implica risco de estar mandando pra Meta valores ainda mais distorcidos do que sem calibração (se a função de calibração não generaliza). Detalhes técnicos completos (5 fases, decisões de design, caminho mínimo alternativo) em [PLANO_REFACTOR_MLOPS.md](PLANO_REFACTOR_MLOPS.md) **DT-20**. Sequência operacional em [EVENTOS_E_DECIS_PLANO.md](EVENTOS_E_DECIS_PLANO.md) seção "Pendência crítica antes do bloco F". Esforço estimado: ~1h com infraestrutura existente; depende de query no Railway pra montar dataset (leads recentes × vendas matched).
- **Bloco A (referência apenas).** Estabilização do fan-out atual de cópia HQ em pixels adicionais cliente-level (atende pedido do dono em 2026-06-07). Canary `smart-ads-api-00680-jez` em 0% tráfego com gates B/D/C.1/C.2 passados. Promoção a 100% conduzida em terminal paralelo — não tocar daqui pra evitar conflito.
