...
Colocar na frente do título dos bugs aqueles que tiveram grande impacto financeiro, inteligência ou performance. 

---

## Erro 1 — Feature de tráfego zerada (`Medium_Linguagem_programacao`)

**Janela:** 26/mar → 13/abr (~18 dias)

Explicar da forma mais simples e rápida possível o que é uma feature.
...
## Erro 2.1 — Idade e salário não chegaram ao modelo (após rollback do modelo)
...
**Insight central:** mesma natureza do Erro 2.2 — o bug reduziu o poder discriminativo do modelo, embaralhando a ordenação. O dano direto medido (~R$ 800) é só o efeito imediato; a degradação acumulada da otimização Meta durante a janela (sinal contaminado chegando ao algoritmo) não está quantificada e soma ao dano total.

---

## Erro 2.2 — Idade e salário não chegaram ao modelo (A/B reativado)
...
## Erro 3 — Evento LQ enviado sem valor por 7 dias
...
**Insight central:** todo o gasto pode ser atribuído como desperdiçado — sem evento aprovado, a Meta não recebia nenhum sinal de otimização, então rodou só por entrega bruta. Diferente do Erro 3 (onde havia evento mas sem valor), aqui não havia evento algum.

---

## Erro 5 — Evento LQ enviado com cobertura ampla e valor superestimado
...
**Resultado (descritivo, sem medição direta):** o algoritmo de otimização da Meta passou a buscar um perfil de lead mais amplo e a "remunerar" cada conversão com um valor inflado. A audiência das campanhas degradou progressivamente:

O ROAS dos LFs do período ficou significativamente abaixo dos LFs limpos (LF44/45), mas o período coincide com o período de março em que a meta declarou ter passado por instabilidade no sistema de anúncios..

**Insight central:** a criação do novo pixel pode ter ajudado a reverter esse erro mais rapidamente. 

---

## Erro 6 — D9 invisível para a Meta (~2 meses, até 15/mar)
...
## Erro 7 — Lista de origens de tráfego chegou vazia em produção
...
**Insight central:** janela curta em produção (~2-3 dias). Natureza similar ao Erro 1 (feature de origem zerando), mas com features distintas afetadas. Sem medição direta de dano financeiro pela curta duração, mas se soma à degradação de sinal vivenciada no mesmo período (sobrepõe Erros 2.2, 3 e 4).
...
- **Verificação após o encoding:** se uma feature importante aparecer zerada em mais de 5% dos leads, o sistema gera alerta e bloqueia. Salvaguarda que faltava nos Erros 1, 2.1, 2.2 e 7. _(21/abr)_
...
- **Alerta para decis sem eventos:** se algum decil (D1–D10) não receber evento por 24h, alerta vermelho. Previne situação como o Erro 6 (D9 invisível por ~2 meses). _(20/abr)_
...
- **Path correto de categorias de origem:** corrigido bug em que a lista de origens válidas chegava vazia em produção (Erro 7). _(02/mai)_
