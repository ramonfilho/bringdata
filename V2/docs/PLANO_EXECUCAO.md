...
- **E7** — **deploy direto a 100% deve ser impossível.** Hoje o `deploy_capi.sh` permite o modo direto (100% tráfego antes do smoke) como caminho normal e `--no-traffic` é uma flag opcional. Mudança: **remover a possibilidade de deploy direto**. O canary com gate E3 (smoke + `/feature-report`) passa a ser o **único** caminho do script. Sem flag de escape, sem `--full-traffic`, sem variável de ambiente que pule. Se o operador quiser bypassar (em emergência), tem que rodar `gcloud run services update-traffic` na mão — fora do script — assumindo o risco explicitamente. **Por que estrito:** a sessão 02/05 mostrou que o gate E3 só pegou os bugs do Medium/Source/telefone porque eu adicionei DEPOIS que o problema apareceu; se um bug similar aparecer no futuro e alguém deployar direto por hábito, o detector existe mas não é exercitado. Local: `api/deploy_capi.sh` (remover `NO_TRAFFIC=false` default, remover branch de tráfego direto, simplificar `deploy_to_cloud_run` pra sempre criar com `--no-traffic --tag` e rodar smoke). ~30 linhas (subtração).
...
- **M2 — `Medium_Linguagem_programacao` extinto.** Categoria saiu da campanha do gestor há semanas. Atualmente jan30 espera essa coluna no feature_registry (importância 5.31%) e ela vem missing em todo poll → T1-10 ERROR crônico. Drift legítimo, não tem fix de encoding. **Decisão pendente:** (a) aceitar como esperado e silenciar T1-10 pra essa coluna específica até o retreino, ou (b) retreinar sem ela. Opção (b) sai automaticamente quando rodar próximo retreino com data mais recente — a feature deixa de aparecer no top-10.
